...
class Logger {
  ...
  private getLogLevel(): LogLevel {
    ...
    switch (envLevel) {
      ...
      default: return isDevelopment() ? LogLevel.DEBUG : LogLevel.INFO;
    }
  }

  private shouldEnableConsole(): boolean {
    // Only enable console logging in development or when explicitly enabled
    if (process.env.FORCE_CONSOLE_LOGGING === 'true') return true;
    if (process.env.DISABLE_CONSOLE_LOGGING === 'true') return false;
...
export const log = {
  ...
  warn: (message: string, metadata?: Record<string, any>, error?: Error) => logger.warn(message, metadata, error),
  ...
  http: (method: string, url: string, statusCode?: number, duration?: number) => 
    logger.logHttpRequest(method, url, statusCode, duration),
