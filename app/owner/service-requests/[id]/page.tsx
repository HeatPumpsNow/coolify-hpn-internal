...
export default function OwnerServiceRequestDetailPage() {
  ...
  const fetchServiceRequestSummary = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/owner/service-requests/${serviceRequestId}`, {
        credentials: 'include'
      });
  ...
  const openServicePortal = () => {
    ...
    const url = authToken 
      ? `${baseUrl}?token=${authToken}&source=owner-portal`
      : `${baseUrl}?source=owner-portal`;
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {serviceRequest.technicianName && (
                <div className="flex items-center text-gray-700">
                  ...
                  <span>Assigned to {serviceRequest.technicianName}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">
              As the owner, you have full administrative access to view, edit, and manage all aspects 
              of this service request. Access work records, communications, photos, and complete service history.
            </p>
            
            <button
              onClick={openServicePortal}
              className="bg-purple-600 hover:bg-purple-700 text-white font-semibold py-3 px-8 rounded-lg transition-colors duration-200 flex items-center space-x-2 mx-auto"
            >
            ...
            <p className="text-sm text-gray-500 mt-4">
              Opens in a new window with full owner permissions
            </p>
          </div>

          {/* Owner Features */}
          <div className="mt-8 bg-gray-50 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Owner Capabilities</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
              <div className="flex items-center">
                <CheckCircle className="h-4 w-4 mr-2 text-green-500" />
                <span>View all work records and activities</span>
              </div>
              <div className="flex items-center">
                <CheckCircle className="h-4 w-4 mr-2 text-green-500" />
                <span>Add work records and updates</span>
              </div>
              <div className="flex items-center">
                <CheckCircle className="h-4 w-4 mr-2 text-green-500" />
                <span>Update service request status</span>
              </div>
              <div className="flex items-center">
                <CheckCircle className="h-4 w-4 mr-2 text-green-500" />
                <span>Access all customer communications</span>
              </div>
              <div className="flex items-center">
                <CheckCircle className="h-4 w-4 mr-2 text-green-500" />
                <span>Assign/reassign technicians</span>
              </div>
              <div className="flex items-center">
                <CheckCircle className="h-4 w-4 mr-2 text-green-500" />
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            <div className="flex justify-center space-x-4">
              <button
                onClick={() => router.push('/owner/service-requests')}
